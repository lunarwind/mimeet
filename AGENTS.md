# MiMeet — Codex CLI 專案規則

> 本檔案供 OpenAI Codex CLI 讀取。請放在專案根目錄並命名為 `AGENTS.md`。
> 目的：讓 Codex 在修改 MiMeet 專案時遵守既有架構、部署、安全、文件與稽核規範。

## Codex CLI 使用原則

### 工作模式

- Codex 必須先理解任務範圍，再修改檔案；不確定時先讀相關程式碼與 `docs/` 規格。
- 優先做最小可驗證修改，不做與任務無關的大型重構。
- 每次修改後，必須列出：
  - 修改了哪些檔案
  - 為什麼修改
  - 執行過哪些檢查或測試
  - 還有哪些未執行或失敗的檢查
- 不要只根據檔名猜測行為；需要先打開實作、路由、型別、測試或部署腳本確認。
- 不要自行編造不存在的指令、檔案、環境變數或服務名稱。

### 命令執行規則

- 執行破壞性命令前必須明確說明原因，例如：`rm -rf`、`docker compose down -v`、`migrate:fresh`、`git reset --hard`、`git clean -fd`。
- 不要自動修改 production；本規範目前只允許 staging 流程。
- 不要把 secret、完整 `.env`、service account JSON、token 或密碼印到終端輸出。
- 需要驗證 secret 檔案時，只能檢查路徑、大小、權限、key 是否存在，或顯示非敏感欄位。
- 若使用 Codex CLI 的自動編輯，修改完成後必須用 `git diff` 檢查實際變更。

### Repo 指引檔位置

- 專案層級指引檔：`<repo-root>/AGENTS.md`
- 若子目錄未來有更細規範，可新增子目錄 `AGENTS.md`，但不得與根目錄規範衝突。
- 若任務只涉及文件，仍需遵守 commit、部署與敏感檔案規則。
...
- 2026-04-25：AGENTS.md layer 2 整合中，docker-compose.staging.yml 改動「報告
  ✅ 但實際沒套用」——因為該檔案在 .gitignore 中，git push 不會同步。教訓：
  改動敏感檔案後必須直接 grep ground truth 驗證，不能只看「Codex CLI 報告」
...
- [ ] 本地跑 `bash scripts/pre-merge-check.sh` 全部 pass（守護條目清單見 `docs/DEV-013_Pre-merge-check_規格書.md`）
- [ ] 後端 model 涉及 datetime 欄位時，確認 `$casts` 已宣告（避免 toISOString() on string 500），且 mutator 操作前先 `->copy()` 避免污染 model attribute（14af 守護）
- [ ] 前端 render function 對 nullable 欄位使用 optional chaining（`?.name` 而非 `.name`）
- [ ] 前端 transformer 不應 hardcode `field: null,`（除非加註解說明為刻意 fallback；14ag 警告守護）
...
- **2026-04-26**：AGENTS.md 整合重寫 layer 2，補上腳本化與 worker 健康檢查盲點修復
...
3. **事後補文件**：48 小時內在 SESSION_SUMMARY 新增條目，內容含：
   ...
   - 與標準流程的差異
   - 如何避免未來再次例外（流程是否需修正）
4. **流程修正**：若例外情境會重複發生，必須更新 AGENTS.md 將其納入標準流程
...
4. **健康檢查守護**
   - 禁止腳本/AGENTS.md 出現失效的健康檢查指令（如 14p 禁止 supervisorctl）
...
若有任何檢查沒有執行，不得寫「全部完成」；必須明確標示未執行原因。

## Codex CLI 修改前快速清單

開始改程式前，先回答下列問題：

- 這次變更是否涉及 API contract / DB schema / queue / cache / build 流程？
- 是否需要同步更新 `docs/` 規格？
- 是否碰到 `.env`、`docker-compose*.yml`、service account 或其他未進 git 的敏感檔案？
- 是否需要更新 `scripts/pre-merge-check.sh` 防止退化？
- 是否需要 staging rollback 計畫？
...
| `docs` | 純文件改動（規格書、AGENTS.md、SESSION_SUMMARY）|
...
- `docs: align DEV-006 references in AGENTS.md`
