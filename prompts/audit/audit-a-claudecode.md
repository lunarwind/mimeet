# Audit-A Round 3 — 認證與身份驗證模組（Claude Code 本機版）

> 你是 Claude Code，運行於本機 mimeet repo 工作目錄。
> 先讀 `prompts/audit/_common.md`，再讀本檔特定章節，依序執行。

---

## 0. 任務摘要

對本機 `develop` 分支的「認證與身份驗證」模組執行第三輪稽核，
產出 `docs/audits/audit-A-{今天日期}-claudecode.md`。

**重點**：本輪是 Round 3，必須對 Round 1（claudecode 兩份）+ Round 2（codex 一份）的
所有 issue 做回歸測試，依 `_common.md` §0「回歸判定方法」嚴格判定狀態。

**特別提醒**：你是本機執行，技術上能改 backend/frontend/admin，但本任務**只允許新增 audit 報告檔**。
若發現 critical issue，記錄在報告 §4 行動優先序，不要動手修——修 code 開另一個 session。
...
**重要**：跑 audit 前必須跟 origin/develop 同步。如果不同步，先 `git pull` 再開始。
...
- `docs/audits/audit-A-20260427-codex.md` — Round 2

**必須做的事**：

1. 用 `view` 工具讀完所有三份報告的「Issues 索引」與「Issue 詳情」區段
...
**特別注意**：Round 1（你前輩 claudecode 跑的）與 Round 2（codex 跑的）對同 issue
可能結論不同。若有分歧，本輪採最新 commit 的實況為準，並在備註欄注明：

> ⚠️ 與 #X-NNN（agent A）結論不同，本輪採 claudecode Round 3 評估
...
```
docs/audits/audit-A-{今天日期}-claudecode.md
```
...
- [ ] 每個 issue 引用的程式碼是實際讀檔出來的
- [ ] 每個 grep 命令的輸出在報告中至少出現一次
- [ ] 每個 issue 都有 file:line + Option A/B + 推薦
- [ ] Symmetric ≥ 10 條
- [ ] 報告檔名 `audit-A-{YYYYMMDD}-claudecode.md`
...
```bash
git add docs/audits/audit-A-{YYYYMMDD}-claudecode.md
...
**不要 push、不要 merge**。等人類審查報告品質後才繼續。

---

## 12. 完成後

報告完成且 commit 後，**停下，不要自動進行下一個 audit**。
等人類確認品質後告訴你下一步。
