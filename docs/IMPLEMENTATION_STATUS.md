...
| F-phone-change | 手機驗證強化 + 換號流程(PR-3 2026-05-08) | ✅ PhoneService(unique+blacklist+race+atomic) + PhoneConflictException + PhoneChangeResult + PhoneChangeHistory + PhoneChangeController(3-step OTP) + AuthController::verifyPhoneSend/Confirm 移除 user-controlled phone 參數 | ✅ PhoneChangeView + utils/mask.ts(deprecated) + VerifyView/RegisterView 改用 authStore.user.phone + 不再傳 phone 給 verify endpoints | ✅ | 14ao~14ar guards;phone_change_histories 表;PR-4 反轉:authStore.user.phone 為 raw,前端視場景決定要不要 mask(目前無 caller) |
| F-dataset-cleanup | mimeet:reset 表清單同步 + 移除測試資料集匯入(PR-Dataset-Cleanup 2026-05-09) | ✅ ResetToCleanState::TRUNCATE_TABLES 補 registration_blacklists/phone_change_histories(28 張) + public const + 移除 SeedTestDataset/TestDataSeeder/9 個 Test*Seeder + DatasetController::seed() + dataset/seed route + --with-test-data flag;DatasetController::stats 業務表 9→24 張對齊 reset 範圍 | ✅ admin SystemSettingsPage 移除「匯入測試資料集」card + freshMode state | ✅ | 14ax guard:每張 migration `Schema::create` 表必須在 TRUNCATE_TABLES 或 PRESERVE whitelist;新增 migration 漏分類即 merge fail。Phase 2 staging `mimeet:reset --force` 清空 PR-2/PR-3 殘留 |
