...
> **v1.4 更新 — 個人資料頁詳細資料區**：
> 操作按鈕下方、關於我上方新增「詳細資料」區塊。採 2 欄 grid 卡片，顯示非敏感 F27 欄位：
...
Step 1：基本資料
  進度指示：「1 / 3」+ 步驟點
  欄位：
    ● 性別（必選）：「生理男」/「生理女」兩個大按鈕卡片
          生理男配圖：男性符號 icon；生理女配圖：女性符號 icon
          選中：#F0294E 邊框 + 淺粉紅背景
    ● 暱稱（2-20字）
      右側「確認可用」按鈕：點擊呼叫 POST /api/v1/auth/check-nickname，
      結果顯示在欄位下方（綠 ✓「暱稱可用」/ 紅 ✗「此暱稱已被使用，請換一個」）。
      用戶任何輸入修改即清掉結果。檢查狀態不影響「下一步」可否進入
      （register 端點仍會二次驗證並 422 攔截）。
