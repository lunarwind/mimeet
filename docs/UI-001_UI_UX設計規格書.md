...
**文檔版本：** v1.2  
**建立日期：** 2026年3月  
**最後更新：** 2026年4月（v1.2 — 新增全域 CSS 變數規範、修正 scoped 樣式限制說明）  
...
> **QR 掃碼驗證頁（`/app/dates/scan`）— v1.3 實作規格**：
> 全螢幕深色背景，含相機掃碼框（4 角粉紅邊框）。
> 掃碼成功後流程：
> 1. 自動呼叫 `navigator.geolocation.getCurrentPosition()`（10 秒逾時）
> 2. GPS 取得：顯示「📍 GPS 定位成功」（綠色），座標送至後端
> 3. GPS 拒絕：顯示「📍 GPS 未取得（加分較少）」（橙色），仍送驗證
> 4. 僅一方掃碼：顯示「⏳ 已掃碼，等待對方」
> 5. 雙方完成：顯示「✅ 約會驗證成功！誠信分數 +5」（GPS 通過）或 +2（未通過）
...
```
主色：使用 Ant Design 預設藍色（#1677FF），與前台有所區隔
...
Tab 3：資料庫設定（super_admin 專屬）
  表單：主機 / Port / 資料庫名 / 使用者名稱 / 密碼（****）
  [🔌 測試連線] 按鈕 → 顯示連線狀態和回應時間
  [儲存設定] 按鈕（需再次輸入管理員密碼）
  ⚠️ 警告：「變更後需重啟容器才完全生效（約 30 秒）」
...
Tab 7：系統參數（super_admin，原有的誠信分數規則等）
```
